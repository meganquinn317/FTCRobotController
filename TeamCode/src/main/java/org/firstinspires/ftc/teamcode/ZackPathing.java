...
@Autonomous(name="ZackPathing", group="")

public class ZackPathing extends LinearOpMode {
    ...
    static final float TURN_SPEED = 0.6f;  // need to check this in robotTurn() method!! 1 is max.
    ...
    public void runOpMode() throws InterruptedException {
        ...
        telemetry.addData("Status", "Initialized");
        telemetry.addData("Initial Heading: ", desiredHeading);
        telemetry.update();
...
//                errorHeading = turnAngle - headingStraight;
    ...
    private void turnToHeading(float speed) {
        ...
        if (Math.abs(desiredHeading - actualHeading)%360 < allowableHeadingDeviation) {
            telemetry.addData("Bailing: ", "within acceptable deviation");
            telemetry.update();
            sleep(3000);
            return;
        }
        // If we have called the function but are already turning to slowly
        // just return.
        if (Math.abs(speed) < .1f) {
            telemetry.addData("Bailing: ", "speed to low");
            telemetry.update();
            sleep(3000);
            return;
        }
        ...
        if ((desiredHeading - actualHeading) > 0 || (desiredHeading - actualHeading) < -180) {
            speed *= -1f;
            isACW = true;
            telemetry.addData("Direction: ", "we are turning ACW");
        } else {
            isACW = false;
            telemetry.addData("Direction: ", "we are turning CW");
        }
        telemetry.update();
        sleep(3000);
        ...
        RB.setPower(-speed);
        if (isACW) {
            while ((desiredHeading - actualHeading)%360 > allowableHeadingDeviation) {
                actualHeading = (float) getHeading(AngleUnit.DEGREES);
            }
            resetEncoders();
            actualHeading = (float) getHeading(AngleUnit.DEGREES);
            if ((desiredHeading - actualHeading)%360 < -allowableHeadingDeviation) {
